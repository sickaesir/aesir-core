...
namespace HidLibrary
{
    public class HidDevice : IDisposable
    {
        ...
        public bool WriteFile(byte[] inputBuffer)
        {
            if (safeReadHandle == null)
                safeReadHandle = OpenHandle(_devicePath, true);
            ...
            try
            {
                byte[] buffer = new byte[inputBuffer.Length + 1];
                buffer[0] = 0;
                Array.Copy(inputBuffer, 0, buffer, 1, inputBuffer.Length);
                return NativeMethods.WriteFile(safeReadHandle.DangerousGetHandle(), buffer, (uint)buffer.Length, out uint bytesWritten, IntPtr.Zero);
            }
            catch(Exception)
            {
                return false;
        ...
        public bool WriteFeatureData(byte[] data)
        {
            ...
            if (NativeMethods.HidD_SetFeature(safeReadHandle, data, data.Length))
                return true;
            else
                return false;
